...
class FeaturePositionalFusion(nn.Module):
    def __init__(self, in_dim, out_dim, pos_dim):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.pos_dim = pos_dim
        self.fusion = nn.Conv1d(in_dim, out_dim, kernel_size=1, bias=True)
    # def forward(self, x, pos_enc):
    def forward(self, x):
        # x: (H*W, in_dim), pos_enc: (H*W, pos_dim)
        x = x.unsqueeze(-1)  # (H*W, in_dim, 1)
        fused = self.fusion(x)  # (H*W, out_dim, 1)
        fused = fused.squeeze(-1)  # (H*W, out_dim)
        # pos_enc: (H*W, pos_dim)
        # 重み付け: fused * pos_enc.sum(dim=1, keepdim=True) など
        # ここでは単純にfusedを返す
        return fused
...
coords = coords.reshape(-1, encoding_dim_x + encoding_dim_y) # (H*W, pos_dim)
...
optimizer = optim.Adam([*netB.parameters(), *fusion.parameters(), Fscr], lr=1e-3)
...
for epoch in progress_bar:
    ...
    fusion_input = torch.cat([coords, Fscr_repeated], dim=1)  # (H*W, pos_dim+C_in)
    fused = fusion(fusion_input)  # (H*W, C_in)
    model_input = fused.view(H, W, C_in).permute(2, 0, 1).unsqueeze(0)  # (H*W, C_in+encoding_dim_x+encoding_dim_y)
